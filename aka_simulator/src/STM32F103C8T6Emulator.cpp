...
STM32F103C8T6Emulator::STM32F103C8T6Emulator()
    : uc_engine_(nullptr), entry_point_(0), code_hook_handle_(0), invalid_mem_hook_handle_(0), boot_mode_(BootMode::Flash)
{
}
...
bool STM32F103C8T6Emulator::setupMemoryRegions()
{
    ...
    {
        err = uc_mem_map(uc_engine_, BOOT_BASE, FLASH_SIZE, UC_PROT_READ | UC_PROT_EXEC);
        if (err != UC_ERR_OK)
        {
            std::cerr << "Failed to map BOOT alias region: " << uc_strerror(err) << std::endl;
            return false;
        }
    }
    else if (boot_mode_ == BootMode::SRAM)
    {
        err = uc_mem_map(uc_engine_, BOOT_BASE, SRAM_SIZE, UC_PROT_READ | UC_PROT_EXEC);
        if (err != UC_ERR_OK)
        {
            std::cerr << "Failed to map BOOT alias region: " << uc_strerror(err) << std::endl;
            return false;
    ...
    // System Memory
    err = uc_mem_map(uc_engine_, SYSTEM_MEMORY_BASE, SYSTEM_MEMORY_SIZE, UC_PROT_READ | UC_PROT_EXEC);
    if (err != UC_ERR_OK)
    {
    ...
    // Peripheral Mapping (mock for MMIO)
    auto map_periph = [&](uint32_t base)
    {
        err = uc_mem_map(uc_engine_, base, BLOCK_SIZE, UC_PROT_READ | UC_PROT_WRITE);
        if (err != UC_ERR_OK)
        {
            std::cerr << "Failed to map peripheral block at 0x" << std::hex << base << ": " << uc_strerror(err) << std::endl;
            return false;
        }
        return true;
    };
    ...
    // APB1 peripherals
    const uint32_t apb1_bases[] = {
        APH1_TIM2_BASE, APH1_TIM3_BASE, APH1_TIM4_BASE, APH1_RTC_BASE, APH1_WWDG_BASE,
        APH1_IWDG_BASE, APH1_SPI2_BASE, APH1_USART2_BASE, APH1_USART3_BASE, APH1_I2C1_BASE,
        APH1_I2C2_BASE, APH1_USB_BASE, APH1_bxCAN_BASE, APH1_BKP_BASE, APH1_PWR_BASE};
    ...
    // APB2 peripherals
    const uint32_t apb2_bases[] = {
        APH2_AFIO_BASE, APH2_EXTI_BASE, APH2_GPIOA_BASE, APH2_GPIOB_BASE, APH2_GPIOC_BASE,
        APH2_GPIOD_BASE, APH2_GPIOE_BASE, APH2_ADC1_BASE, APH2_ADC2_BASE, APH2_TIM1_BASE,
        APH2_SPI1_BASE, APH2_USART1_BASE};
    ...
    // AHB peripherals
    const uint32_t ahb_bases[] = {
        AHB_DMA_BASE, AHB_RCC_BASE, AHB_FLASH_BASE, AHB_CRC_BASE};
